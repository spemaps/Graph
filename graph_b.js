 ...
 */
  ...
  function init () {
    ...
    var edge_select = document.getElementById('edge');
    edge_select.addEventListener('click', edge_change, false);

    var node_select = document.getElementById('node');
    node_select.addEventListener('click', node_change, false);
...
//
 ...
 //The event handler for any changes made to the tool selector.
  function edge_change (ev) {
    if (tools[edge]) {
      tool = new tools[edge]();
    }
  }

  function node_change(ev){
    if(tools[node])
      tool = new tools[node]
  }
